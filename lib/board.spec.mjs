...
export function isValid_returns_true_for_a_valid_board() {
  const validBoard = new Board('1'.padEnd(81, ' '))
  assert.deepEqual(validBoard.isValid(), true, 'true for a valid board')
}

export function isValid_returns_false_for_a_board_with_errors() {
  const invalidBoard = new Board()
  invalidBoard.cells = '11'.padEnd(81, ' ')
  assert.deepEqual(invalidBoard.isValid(), false, 'returns false for an invalid board')
}