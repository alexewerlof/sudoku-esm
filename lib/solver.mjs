...
function allEmptyCellsWithPossibilities(board) {
  const allEmptyCells = board.findAllEmptyCells()
  const ret = new Array(allEmptyCells.length)
  for (let i = 0; i < allEmptyCells.length; i++) {
    const [ x, y ] = allEmptyCells[i]
    const possibilities = board.getPossibilities(x, y)
    if (possibilities.length === 0) {
      return
    }
    ret[i] = ({ x, y, possibilities })
  }
  return ret
}
  ...
  do {
    ...
    guesses.forEach(p => {
        if (p.possibilities.length === 1) {
          board.setCell(p.x, p.y, p.possibilities[0])
          isBoardChanged = true
        }
      })
