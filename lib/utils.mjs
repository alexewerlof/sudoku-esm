...
export function memoizedSetCharAtOld(str, index, newChar) {
...
export function memoize1(fn) {
  const dic = {}
  function memoizedFn(key) {
    const val = dic[key]
    if (val) {
      return val
    }
    return dic[key] = fn(key)
  }
  return memoizedFn
}
...
export function memoize2(fn) {
  const dic = {}
  function memoizedFn(a, b) {
    const key = a + b
    const val = dic[key]
    if (val) {
      return val
    }
    return dic[key] = fn(a, b)
  }
  return memoizedFn
}
...
export function memoize3(fn) {
  const dic = {}
  function memoizedFn(a, b, c) {
    const key = a + b + c
    const val = dic[key]
    if (val) {
      return val
    }
    return dic[key] = fn(a, b, c)
  }
  return memoizedFn
}
