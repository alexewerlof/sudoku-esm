...
function getGuessQuality(freq, { possibilities }) {
  ...
  // The best quality guess is the one that has the most common numbers and has the biggest number of different possibilities
...
export function getGuess(board) {
  ...
  for (let r = board.indexOf(0); r !== -1; r = board.indexOf(0, r + 1)) {
    ...
    for (let p = 0; p < possibilities.length; p++) {
      guessForPossibility[possibilities[p]].push(guess)
    }
    guesses.push(guess)
  }
  ...
  for (let i = 1; i < 10; i++) {
    if (guessForPossibility[i].length === 1) {
      const guess = guessForPossibility[i][0]
      guess.possibilities.length = 1
      guess.possibilities[0] = i
      return guess
    }
  }
