...
const memoizedSetCharAtDic = {}
export function memoizedSetCharAt(str, index, newChar) {
  const key = str + index + newChar
  const val = memoizedSetCharAtDic[key]
  if (val) {
    return val
  }
  return memoizedSetCharAtDic[key] = setCharAt(str, index, newChar)
}
...
const memoizedGetPossibilitiesDic = {}
export function memoizedGetPossibilities(board, r) {
  const key = board + r
  const val = memoizedGetPossibilitiesDic[key]
  if (val) {
    return val
  }
  return memoizedGetPossibilitiesDic[key] = getPossibilities(board, r)
}
